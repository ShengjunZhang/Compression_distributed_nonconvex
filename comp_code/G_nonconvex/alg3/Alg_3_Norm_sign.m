function [xminuxbar, sq_grad, transmit] = Alg_3_Norm_sign(L_aug, x, dn,N,gc,lambda,aalpha, features, labels,bs, iter_num, eta, alpha, beta, s0, gamma)

    fprintf('Alg_3_Norm_sign\n');
    ...
    for i = 1: N
        q_0( (i-1)*dn+1 : i*dn) = Norm_sign(x0(:, i) / s0);
    end
    ...
    for t = 2:iter_num     
        ...
        s = s0 * gamma^(t-1);
        for i = 1:N
            q_0( (i-1)*dn+1 : i*dn) = Norm_sign((x_comp(:, i) - x_hat_comp(:, i))/s);
        end
