function [xminuxbar, sq_grad, transmit] = Alg_3_Norm_signE(L_aug, x, dn,N,gc,lambda,aalpha, features, labels,bs, iter_num, eta, alpha, beta, s0, gamma)
    ...
    for i = 1: N
        q_0( (i-1)*dn+1 : i*dn) = Norm_sign(x0(:, i));
    end
    x_hat(:, 2) = x_hat(:, 1) + q_0;
    ...
    for t = 2:iter_num
        ...
        for i = 1:N
            q_0( (i-1)*dn+1 : i*dn) = Norm_sign(x_comp(:, i) - x_hat_comp(:, i));
        end
